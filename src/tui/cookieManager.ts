  ...
  while (true) {
    ...
    const domainChoices = [
      ...domains.map((d) => {
        ...
        return {
          name: d,
          message: `${chalk.cyan(d)}  ${chalk.dim(`(${count} cookie${count !== 1 ? "s" : ""})`)}`,
        };
      }),
  ...
  while (true) {
    ...
    const { action } = await prompt<{ action: string }>({
      ...
      choices: [
        ...
        {
          name: "delete_all",
          message: chalk.red("Delete all cookies for this domain"),
          disabled: cookies.length === 0,
        },
    ...
    if (action === "add_kv") {
      ...
      const { confirmed } = await prompt<{ confirmed: boolean }>({
        ...
        name: "confirmed",
        message: chalk.red(
          `Delete all ${cookies.length} cookie(s) for ${domain}? This cannot be undone.`,
        ),
        initial: false,
      });
      if (confirmed) {
