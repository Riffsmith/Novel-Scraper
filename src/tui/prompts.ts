...
export async function gatherConfig(
  ...
  if (hasExclusions) {
    ...
    cfg.excludeSelectors = r.exclusionList
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
  }
  ...
  disp.info(
    `Output  : ${chalk.cyan(ro1.outputDir + "/" + ro2.outputFilename)}`,
  );
  console.log("");
...
//
//  This is the "customize everything" path. Most people scraping a
//  supported site won't need it — see buildQuickAutoConfig() below for the
//  fast path that skips straight to a single confirmation.
// ═══════════════════════════════════════════════════════════════════════════
export async function gatherAutoConfig(
  ...
  if (hasExclusions) {
    ...
    excludeSelectors = r.exclusionList
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
  }
...
// ═══════════════════════════════════════════════════════════════════════════
//  buildQuickAutoConfig — the fast path for auto-scraping.
//
//  Composes a complete ScraperConfig directly from adapter defaults, any
//  saved site profile, and global app settings — with zero prompts. This is
//  what lets the "quick" auto-scrape flow in index.ts get from a pasted URL
//  to a running scrape after exactly two confirmations: one to accept these
//  auto-filled settings, and one to actually start.
//
//  Anything a site profile has saved takes priority over the adapter's
//  built-in defaults, since a profile reflects settings the user already
//  confirmed worked on a previous scrape of the same domain.
// ═══════════════════════════════════════════════════════════════════════════
