import PQueue from "p-queue";
import type { Browser, Cookie } from "playwright";
import type {
  Chapter,
  QueueTask,
  ScrapeError,
  ScraperConfig,
  AppConfig,
} from "../types.js";
import {
  createStealthContext,
  createPage,
  randomDelay,
} from "../scraper/browser.js";
import type { BrowserContext } from "playwright";
import { scrapeChapter, SecurityChallengeError } from "../scraper/chapter.js";
import logger from "../logger/index.js";
import { createProgressBar } from "../tui/display.js";
import chalk from "chalk";
const CHALLENGE_BACKOFF_MS = 45_000; // longer pause when we got flagged, not just erred

export interface QueueResult {
  chapters: Chapter[];
  errors: ScrapeError[];
}

// ── Resume support ──────────────────────────────────────────────────────────
// Passed in by index.ts when a scrape is starting fresh (checkpoint present
// but previousChapters empty) or continuing an earlier session
// (previousChapters populated). Chapters already present in
// previousChapters are never re-scraped — their ORIGINAL 1-based `.index`
// is what determines which task gets skipped, so numbering (and therefore
// EPUB filenames/spine order) stays identical to the first attempt even
// though `urls` here is always the FULL original list, not just the
// remaining ones.
export interface ResumeCheckpoint {
  previousChapters?: Chapter[];
  // Called after each chapter attempt resolves (success or final failure),
  // throttled to at most once every few seconds, plus once, unconditionally,
  // right before this function returns. Intended for persisting a session
  // checkpoint to disk — see sessions/store.ts.
  onProgress?: (snapshot: {
    chapters: Chapter[];
    errors: ScrapeError[];
  }) => void;
}

const CHECKPOINT_SAVE_INTERVAL_MS = 4_000;

// ═══════════════════════════════════════════════════════════════════════════
//  Task queue – manages N concurrent browser contexts, retries failed
//  tasks up to maxRetries, and streams progress to the TUI.
// ═══════════════════════════════════════════════════════════════════════════
export async function runScrapeQueue(
  browser: Browser,
  urls: string[],
  config: ScraperConfig,
  cookies?: Cookie[], // pre-loaded from the cookie store for this domain
  appCfg?: AppConfig, // global settings — maxRetries, waitUntil, navTimeout
  checkpoint?: ResumeCheckpoint,
): Promise<QueueResult> {
  const { concurrency, delayMin, delayMax } = config;
  const maxRetries = appCfg?.maxRetries ?? 3;
  const waitUntil = appCfg?.waitUntil ?? "domcontentloaded";
  const navTimeoutMs = appCfg?.navigationTimeoutMs ?? 30_000;

  // ── Pre-allocate result slots, seeded with anything already scraped ────
  const slots: (Chapter | null)[] = new Array(urls.length).fill(null);
  const alreadyDone = new Set<number>(); // 0-based positions into `urls`/`slots`
  for (const ch of checkpoint?.previousChapters ?? []) {
    const slotIdx = ch.index - 1;
    if (slotIdx >= 0 && slotIdx < slots.length) {
      slots[slotIdx] = ch;
      alreadyDone.add(slotIdx);
    }
  }

  const errors: ScrapeError[] = [];
  let completed = alreadyDone.size;

  const progressBar = createProgressBar(urls.length);
  if (completed > 0) {
    progressBar.update(completed, { chapter: chalk.dim("resumed") });
  }

  // ── Throttled checkpoint persistence ────────────────────────────────────
  let lastSaveAt = 0;
  const maybePersist = (force = false): void => {
    if (!checkpoint?.onProgress) return;
    const now = Date.now();
    if (!force && now - lastSaveAt < CHECKPOINT_SAVE_INTERVAL_MS) return;
    lastSaveAt = now;
    try {
      checkpoint.onProgress({
        chapters: slots.filter((c): c is Chapter => c !== null),
        errors: [...errors],
      });
    } catch (e) {
      logger.warn(`Checkpoint persistence failed: ${(e as Error).message}`);
    }
  };

  // ── Context pool (one per concurrent slot) ────────────────────────────
  //  Each context gets the same cookie snapshot so every worker is
  //  authenticated from the very first request.
  const ctxPool: BrowserContext[] = [];
  for (let i = 0; i < concurrency; i++) {
    ctxPool.push(await createStealthContext(browser, cookies));
  }
  let ctxIdx = 0;
  const nextCtx = () => ctxPool[ctxIdx++ % ctxPool.length];

  // ── Queue ──────────────────────────────────────────────────────────────
  const queue = new PQueue({ concurrency });

  const processTask = async (task: QueueTask): Promise<void> => {
    const ctx = nextCtx();
    const page = await createPage(ctx);

    try {
      // Jitter before each request
      await randomDelay(delayMin, delayMax);

      const chapter = await scrapeChapter(page, task.url, task.index + 1, {
        contentSelector: config.contentSelector,
        titleSelector: config.titleSelector,
        separateTitle: config.separateTitle,
        excludeSelectors: config.excludeSelectors,
        delayMin,
        delayMax,
        waitUntil,
        navTimeoutMs,
      });

      if (chapter) {
        slots[task.index] = chapter;
      } else {
        // Retry
        if (task.retries < task.maxRetries) {
          task.retries++;
          const backoff = task.retries * delayMax;
          logger.warn(
            `Re-queuing ch.${task.index + 1} (attempt ${task.retries}/${task.maxRetries}) after ${backoff}ms`,
          );
          await new Promise((r) => setTimeout(r, backoff));
          await queue.add(() => processTask(task));
          return;
        }
        errors.push({
          url: task.url,
          error: "No content extracted after max retries",
          retries: task.retries,
        });
      }
    } catch (e) {
      const isChallenge = e instanceof SecurityChallengeError;
      if (task.retries < task.maxRetries) {
        task.retries++;
        const backoff = isChallenge
          ? task.retries * CHALLENGE_BACKOFF_MS
          : task.retries * delayMax;
        logger.warn(
          `${isChallenge ? "Security challenge on" : "Error on"} ch.${task.index + 1} – retrying (${task.retries}/${task.maxRetries}) after ${backoff}ms: ${(e as Error).message}`,
        );
        await new Promise((r) => setTimeout(r, backoff));
        await queue.add(() => processTask(task));
        return;
      }
      errors.push({
        url: task.url,
        error: (e as Error).message,
        retries: task.retries,
      });
      logger.error(`Dropped ch.${task.index + 1}: ${(e as Error).message}`);
    } finally {
      await page.close().catch(() => {
        /* ignore if already closed */
      });
    }

    completed++;
    progressBar.update(completed, {
      chapter: chalk.dim(`ch.${task.index + 1}`),
    });
    maybePersist();
  };

  // ── Enqueue all tasks, skipping anything already scraped in a prior run ─
  const tasks: QueueTask[] = urls
    .map((url, index) => ({
      url,
      index,
      retries: 0,
      maxRetries, // sourced from appCfg.maxRetries, default 3
    }))
    .filter((task) => !alreadyDone.has(task.index));

  await queue.addAll(tasks.map((task) => () => processTask(task)));
  await queue.onIdle();

  progressBar.stop();
  maybePersist(true); // final, unconditional checkpoint

  // ── Tear down context pool ────────────────────────────────────────────
  for (const ctx of ctxPool) {
    await ctx.close().catch(() => {});
  }

  // ── Collect valid chapters in index order (includes anything carried
  //    over from previousChapters, since those already occupy their slots)
  const chapters = slots
    .filter((c): c is Chapter => c !== null)
    .sort((a, b) => a.index - b.index);

  logger.info(`Queue complete: ${chapters.length} ok, ${errors.length} failed`);

  return { chapters, errors };
}
