...
async function resolveNextElement(
  ...
  for (let i = 0; i < locators.length; i++) {
    ...
    try {
      ...
      logger.debug(
        `Locator #${i} (${loc.kind}) error: ${(e as Error).message}`,
      );
    }
  }
  return null;
  ...
  try {
    while (currentUrl && links.length < MAX_CHAPTERS) {
      ...
      try {
        ...
        await randomDelay(
          Math.floor(delayMin * 0.4),
          Math.floor(delayMax * 0.4),
        );
      } catch (e) {
        logger.error(`Navigation failed: ${currentUrl}`, {
      ...
      if (!resolved) {
        const tried = locators
          .map(
            (l, i) =>
              `  ${i === 0 ? "primary  " : `fallback ${i}`}: ${formatLocator(l)}`,
          )
          .join("\n");
        logger.warn(
          `No next-button found at ch.${links.length}.\nTried:\n${tried}`,
        );
        ...
        if (hasFallbacks) {
          ...
          spin.succeed(
            `No more chapters detected. Collected ${links.length} URL(s).`,
          );
        }
        break;
      }
      ...
      const href = await resolved.element
        .getAttribute("href")
        .catch(() => null);
      if (
        href?.trim() &&
        ...
        try {
          ...
          await randomDelay(
            Math.floor(delayMin * 0.3),
            Math.floor(delayMax * 0.3),
          );
          continue;
        } catch {
      ...
      try {
        ...
        logger.error(`Click navigation failed at ${currentUrl}`, {
          error: (e as Error).message,
        });
        break;
      }
    ...
    if (links.length >= MAX_CHAPTERS)
      spin.warn(`Safety limit (${MAX_CHAPTERS}) reached`);
